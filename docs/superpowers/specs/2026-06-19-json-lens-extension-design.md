...
- Provide an optional checkbox to synchronize source and result scrolling.
- Keep row-level copy controls hidden until hover or keyboard focus.
- Support right-click copying for result rows.
...
- Header toolbar: app name, validation status, language switcher, new document, read clipboard, load file, format, save, clear, clear cache, copy all, download, sync scroll, hide/show source.
